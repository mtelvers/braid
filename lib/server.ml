...
let run ~sw ~net ~fs ~listen_addr ~listen_port ~key_file ~cap_file ~opam_repo_path ~cache_dir =
  ...
  let config = Capnp_rpc_unix.Vat_config.create ~secret_key ~net addr in
