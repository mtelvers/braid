...
let run ~sw ~net ~listen_addr ~listen_port ~cap_file ~opam_repo_path ~cache_dir =
  let service = Rpc_service.local ~opam_repo_path ~cache_dir in
  let addr = `TCP (listen_addr, listen_port) in
  let config = Capnp_rpc_unix.Vat_config.create ~secret_key:`Ephemeral ~net addr in
  ...
  Fmt.pr "Server listening on %s:%d, capability file: %s@." listen_addr listen_port cap_file;
